...
config = {
  'corpus':"./Full Dataset/corpus.json",
  'saveFolder':"./Full Dataset/",
  ...
  'modelURL': ["", 'all-distilroberta-v1', "", "/Full Dataset/d2v.model"]
}
...
file = open("./Full Dataset/nodeIDs.json")
...
file = open("./Full Dataset/nodes.json")
...
obj = {
'min_cohashtags': min(co_hashtag_both),
'min_comentions' : min(co_mention_both),
'min_courls' : min(co_url_both),
'max_cohashtags' : max(co_hashtag_both),
'max_comentions' :  max(co_mention_both),
'max_courls': max(co_url_both)
}

min_max_data = json.dumps(obj, indent=4, sort_keys=True, default=str)
with open("./Full Dataset/min_max_info.json", "w") as outfile:
    outfile.write(min_max_data)



file = open("./Full Dataset/corpus.json")
...
model.save("./Full Dataset/d2v.model")


#####################

def save_embeddings(sim_model_idx):
  file = open(config['corpus'])
  ...
  with open(config['saveFolder']+config['similarityModel'][sim_model_idx] +"_embeddings.json", "w") as outfile:
...
####### MAIN FUNC
def get_truth_false_scores(metric, sim_model_idx):
  file = open(config['nodeIDs'])
  nodeIDs = json.load(file)
  file.close()

  file = open(config['nodes'])
  nodesdata = json.load(file)
  file.close() 
  
  file = open(config['saveFolder']+config["similarityModel"][sim_model_idx]+"_embeddings.json", 'rb')
...
def Classify(model, text_embeddings=False):
...
######### MAIN FUNC
def create_graph(threshold, cosines, simidx):
  file = open(config['nodeIDs'])
  nodes = json.load(file)
  file.close()
  ...
  nx.write_gexf(graph, config['saveFolder']+config['similarityModel'][simidx]+"_weighted_graph_"+str(num)+".gexf")
...
def node2vec(sim_model, graph, best_model_config, walk_len, dimensions, pq, threshold):
  ...
  for dim in dimensions:
    for leng in walk_len:
      for pq_test in pq:
        ...
        try:
          model = pickle.load(open(config['saveFolder'] + sim_model + "_n2v_model_" + 
        ...
        if not model:
          ...
          model.wv.save_word2vec_format(config['saveFolder'] + sim_model + "_n2v_embeddings_" + str(dim)+"_" + str(leng)+"_" + str(p_test) + "_" + str(q_test)+"_"+str(threshold))
          
          model.save(config['saveFolder'] + sim_model +  "_n2v_model_" + str(dim)+"_" + str(leng)+"_" + str(p_test) + "_" + str(q_test)+"_"+str(threshold))
          print("saved model")
        c_output = Classify(model)
        if best_model_config["f1-score"] < c_output[4] :
          ...
          with open(config['saveFolder'] + sim_model + "_best_model_config_"+str(threshold)+".json", "w") as outfile:
            outfile.write(best_model)



def run_node2vec_for_graphs(thresholds, best_model_config, simidx):
  for thresh in thresholds:
    print("Running for iteration "+str(thresh))
    num = round(thresh, 5)
    graph = nx.read_gexf(config['saveFolder']+config['similarityModel'][simidx]+ "_weighted_graph_"+str(num)+".gexf")
    ...
    model = node2vec(config['similarityModel'][simidx], graph, best_model_config, walk_len, dim, pq, thresh)
...
for smodel_idx in range(len(simmodels)):
    try:
      file = open(config['saveFolder']+config['similarityModel'][smodel_idx]+"_embeddings.json", "r")
      if(not file):
        save_embeddings(smodel_idx)
      else:
        print(config['similarityModel'][smodel_idx], "emnbeddings found, moving on")
    except Exception as e:
      print("embeddings not found, creating", e)
      save_embeddings(smodel_idx)
    
    ts_alone_dot, fs_alone_dot, s_alone_dot = get_truth_false_scores('dot', smodel_idx)
    ts_alone_cosine, fs_alone_cosine, s_alone_cosine = get_truth_false_scores('cosine', smodel_idx)
    ...
    for z in threshArr:
      ...
      graph = create_graph(z, s_alone, smodel_idx)
    ...
    best_model_config = {
        ...
        "q": 0,
        "Classifiers": "",
        "Sim Model": config['similarityModel'][smodel_idx],
        "f1-score": 0,
    }
    run_node2vec_for_graphs(threshArr, best_model_config, smodel_idx)